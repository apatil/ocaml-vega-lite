...
let enc1 =
  let df = FieldDef.(make `Temporal
    |> timeUnit (`Single (`Local `Year))
    |> field (`String "date"))
  in
  let col = ConditionalLegendFieldDef.(make `Nominal |> field (`String "location")) in
...
let layer1 = CompositeUnitSpec.(make (`Mark `Line) |> encoding enc1)
...
let layer2 = CompositeUnitSpec.(make (`Mark `Line) |> encoding enc2)

let layerSpec = LayerSpec.(make [
    (`Unit layer1);
    (`Unit layer2)
  ])

let jsonSpec = TopLevelRepeatSpec.(make (`Layer layerSpec)
  |> description "Summarized and per year weather information for Seatle and New York."
  |> data (`Url UrlData.(make "data/weather.csv"))
